...
bool bt_mergeFiles(char * outputFileName, size_t nFiles, char ** fileSequence){
	...
	char * outputBuffer = calloc(sizeof(char), allSize+1);
	...
	while(allSize > 0){
		for(size_t i = 0; i < nFiles; i++){
			...
			if(buffer[0] == 0){
				...
				memcpy(outputBuffer + outputPointer, buffer, inputSize);
				outputPointer += inputSize;
			}
		}
	}	

	fwrite(outputBuffer, sizeof(char), outputPointer, output);
...
char ** bt_mergeSequences(size_t nSequences, char *** fileNames){
	...
	pid_t * childProcesses = calloc(sizeof(pid_t), nSequences);
